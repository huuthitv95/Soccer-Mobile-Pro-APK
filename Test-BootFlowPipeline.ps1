...
foreach($name in @('Recover-SoccerUnity.ps1','Test-BootFlow.ps1','Test-BootFlowPipeline.ps1')) {
...
foreach($mode in @('SelfTest','PreflightOnly','RepairExisting','ValidateOnly','ValidateOfflineUI')) {
    $arguments=@{ValidateBootFlow=$true};$arguments[$mode]=$true
    $rejected=$false
...
Write-Host 'Boot pipeline contract passed: parse, five exclusive mode pairs, existing-project refusal.'
