package com.ironsource;

import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import com.unity3d.mediation.LevelPlay;
import java.util.List;
import kotlin.collections.CollectionsKt;
import kotlin.jvm.internal.Intrinsics;
import kotlin.text.StringsKt;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: renamed from: com.ironsource.Mf */
/* JADX INFO: loaded from: classes6.dex */
public final class C11567Mf implements InterfaceC11387Cf {

    /* JADX INFO: renamed from: a */
    private final Handler f25170a;

    /* JADX INFO: renamed from: b */
    private WebView f25171b;

    /* JADX INFO: renamed from: c */
    private final WebView f25172c;

    /* JADX INFO: renamed from: d */
    private final JSONObject f25173d;

    /* JADX INFO: renamed from: e */
    private boolean f25174e;

    /* JADX INFO: renamed from: f */
    private boolean f25175f;

    /* JADX INFO: renamed from: com.ironsource.Mf$a */
    public static final class a extends WebViewClient {

        /* JADX INFO: renamed from: b */
        final /* synthetic */ String f25177b;

        a(String str) {
            this.f25177b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(WebView webView, String str) throws JSONException {
            if (!C11567Mf.this.f25174e && str != null && StringsKt.startsWith$default(str, String.valueOf(this.f25177b), false, 2, (Object) null)) {
                C11567Mf c11567Mf = C11567Mf.this;
                List<? extends Object> listListOf = CollectionsKt.listOf(c11567Mf.f25173d);
                Intrinsics.checkNotNull(listListOf, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                c11567Mf.m26570a(c11567Mf.m26568a("onDataReady", listListOf));
                C12608xf.f33185a.m34186d();
                C11567Mf.this.f25174e = true;
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) throws JSONException {
            Intrinsics.checkNotNullParameter(view, "view");
            Intrinsics.checkNotNullParameter(request, "request");
            Intrinsics.checkNotNullParameter(error, "error");
            C12608xf.f33185a.m34183a(error.toString());
            C11459Gf.f24392a.m25946b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }
    }

    /* JADX INFO: renamed from: com.ironsource.Mf$b */
    public static final class b implements InterfaceC11631Q8 {

        /* JADX INFO: renamed from: b */
        final /* synthetic */ InterfaceC11631Q8 f25179b;

        b(InterfaceC11631Q8 interfaceC11631Q8) {
            this.f25179b = interfaceC11631Q8;
        }

        /* JADX INFO: Access modifiers changed from: private */
        /* JADX INFO: renamed from: a */
        public static final void m26574a(b this$0) {
            Intrinsics.checkNotNullParameter(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.ironsource.InterfaceC11631Q8
        public void onClosed() {
            this.f25179b.onClosed();
        }

        @Override // com.ironsource.InterfaceC11631Q8
        public void onUIReady() {
            if (C11567Mf.this.f25175f) {
                this.f25179b.onUIReady();
            } else {
                C11567Mf.this.f25170a.postDelayed(new Runnable() { // from class: com.ironsource.Mf$b$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        C11567Mf.b.m26574a(this.f$0);
                    }
                }, 500L);
            }
        }
    }

    public C11567Mf(TestSuiteActivity activity, InterfaceC11631Q8 uiLifeCycleListener, JSONObject jsonData, String str) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "uiLifeCycleListener");
        Intrinsics.checkNotNullParameter(jsonData, "jsonData");
        this.f25170a = new Handler(activity.getMainLooper());
        this.f25171b = new WebView(activity);
        this.f25172c = new WebView(activity);
        this.f25173d = jsonData;
        m26563a(activity, uiLifeCycleListener, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: e */
    public static final void m26567e(C11567Mf this$0) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        this$0.f25175f = true;
    }

    /* JADX INFO: renamed from: b */
    public final void m26571b() {
        WebView webView = this.f25171b;
        if (webView != null) {
            if (webView != null) {
                webView.destroy();
            }
            this.f25171b = null;
        }
    }

    /* JADX INFO: renamed from: c */
    public final WebView m26572c() {
        return this.f25172c;
    }

    /* JADX INFO: renamed from: d */
    public final WebView m26573d() {
        return this.f25171b;
    }

    /* JADX INFO: renamed from: a */
    private final void m26563a(TestSuiteActivity testSuiteActivity, final InterfaceC11631Q8 interfaceC11631Q8, String str) {
        WebView webView = this.f25171b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f25172c.getSettings().setJavaScriptEnabled(true);
        this.f25172c.addJavascriptInterface(new C11369Bf(new C12572vf(testSuiteActivity, this.f25170a), new b(interfaceC11631Q8), this), "TestSuiteBridge");
        this.f25172c.setWebViewClient(new a(str));
        if (str != null && str.length() != 0) {
            this.f25172c.loadUrl(str);
        }
        WebView webView2 = this.f25171b;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.f25170a.postDelayed(new Runnable() { // from class: com.ironsource.Mf$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                C11567Mf.m26567e(this.f$0);
            }
        }, 2000L);
        this.f25170a.postDelayed(new Runnable() { // from class: com.ironsource.Mf$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
                C11567Mf.m26562a(interfaceC11631Q8);
            }
        }, 5000L);
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: a */
    public static final void m26562a(InterfaceC11631Q8 uiLifeCycleListener) {
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    @Override // com.ironsource.InterfaceC11387Cf
    /* JADX INFO: renamed from: a */
    public void mo25585a(String callbackName, LevelPlay.AdFormat adFormat, List<? extends Object> args) {
        Intrinsics.checkNotNullParameter(callbackName, "callbackName");
        Intrinsics.checkNotNullParameter(adFormat, "adFormat");
        Intrinsics.checkNotNullParameter(args, "args");
        if (this.f25174e) {
            m26570a(m26568a("onCallbackReceived", CollectionsKt.listOf(callbackName, adFormat.getValue(), args)));
        }
    }

    /* JADX INFO: renamed from: a */
    public final void m26570a(final String script) {
        Intrinsics.checkNotNullParameter(script, "script");
        this.f25170a.post(new Runnable() { // from class: com.ironsource.Mf$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
                C11567Mf.m26560a(this.f$0, script);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: a */
    public static final void m26560a(C11567Mf this$0, String script) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        Intrinsics.checkNotNullParameter(script, "$script");
        this$0.f25172c.loadUrl("javascript:" + script);
    }

    /* JADX INFO: renamed from: a */
    public final String m26568a(String methodName, List<? extends Object> methodArgs) {
        Intrinsics.checkNotNullParameter(methodName, "methodName");
        Intrinsics.checkNotNullParameter(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String strM26442b = C11549Lf.f25085a.m26442b(methodArgs);
        if (strM26442b.length() > 0) {
            return str + "(" + strM26442b + ")";
        }
        return str + "()";
    }

    /* JADX INFO: renamed from: a */
    public final void m26569a() {
        m26571b();
        this.f25172c.destroy();
    }
}
