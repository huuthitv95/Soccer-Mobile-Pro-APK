...
import com.ironsource.C12383n5;
...
public class DeviceInfoReaderBuilder {
    ...
    private JsonFlattenerRules getTsiRequestStorageRules() {
        return new JsonFlattenerRules(Arrays.asList(ShareConstants.WEB_DIALOG_PARAM_PRIVACY, GDPR.GDPR_STANDARD, "framework", C11540L6.f24916G1, "unity", "pipl", "configuration", "user", JsonStorageKeyNames.UNIFIED_CONFIG_KEY), Collections.singletonList("value"), Arrays.asList("ts", "exclude", "pii", "nonBehavioral", "nonbehavioral"));
    }
    ...
    protected JsonFlattenerRules getMediationInfoRules() {
        return new JsonFlattenerRules(Collections.singletonList("mediation"), Collections.singletonList("value"), Arrays.asList("ts", C12383n5.f31533a));
    }
}
