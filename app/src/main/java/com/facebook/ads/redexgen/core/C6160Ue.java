...
/* JADX INFO: renamed from: com.facebook.ads.redexgen.X.Ue */
/* JADX INFO: loaded from: assets/audience_network/classes2.dex */
public class C6160Ue {
     ...
     */
    public static C6160Ue A04(File file) throws IOException {
        try {
            ...
            if (j2 < 0 || j2 > SilenceSkippingAudioProcessor.DEFAULT_MINIMUM_SILENCE_DURATION_US) {
                throw new IOException(String.format(Locale.US, A05(77, 40, 27), Long.valueOf(j2)));
            }
            if (randomAccessFile.length() < A02((int) j2)) {
                throw new IOException(String.format(Locale.US, A05(Sdk.SDKError.Reason.ASSET_FAILED_STATUS_CODE_VALUE, 63, 56), Long.valueOf(j2), Long.valueOf(file.length())));
            }
            long[] jArr = new long[(int) j2];
            for (int i = 0; i < jArr.length; i++) {
                jArr[i] = randomAccessFile.readLong();
            }
            return new C6160Ue(randomAccessFile, jArr, randomAccessFile.length());
    ...
    public C6160Ue(RandomAccessFile randomAccessFile, long[] jArr, long j) throws IOException {
    ...
    public static C6160Ue A03(File file) throws IOException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 33));
        try {
            randomAccessFile.writeLong(-360011992771067903L);
            randomAccessFile.writeLong(C6161Uf.A03);
            randomAccessFile.write(new byte[C6161Uf.A03 * 8]);
            randomAccessFile.getFD().sync();
            return new C6160Ue(randomAccessFile, new long[C6161Uf.A03], randomAccessFile.length());
        } catch (IOException unused) {
