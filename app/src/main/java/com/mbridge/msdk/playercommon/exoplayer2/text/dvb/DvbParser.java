...
final class DvbParser {
    ...
    private static final byte[] defaultMap2To4 = {0, 7, 8, Ascii.f22499SI};
            ...
            if (bits != 240) {
                switch (bits) {
                    ...
                    case 17:
                        ...
                        Paint paint3 = paint;
                        canvas2 = canvas;
                        iPaint2BitPixelCodeString = paint4BitPixelCodeString(parsableBitArray, iArr3, i == 3 ? defaultMap4To8 : null, iPaint2BitPixelCodeString, i4, paint3, canvas2);
                        parsableBitArray.byteAlign();
                        paint = paint3;
                        iArr = iArr3;
                        break;
            ...
            case 17:
                ...
                if (bits2 == subtitleService.subtitlePageId && pageComposition3 != null) {
                    ...
                    if (pageComposition3.state == 0) {
                        regionComposition.mergeFrom(subtitleService.regions.get(regionComposition.f37417id));
                    }
                    subtitleService.regions.put(regionComposition.f37417id, regionComposition);
                }
                break;
            case 18:
                if (bits2 == subtitleService.subtitlePageId) {
                    ClutDefinition clutDefinition = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.cluts.put(clutDefinition.f37415id, clutDefinition);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ClutDefinition clutDefinition2 = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.ancillaryCluts.put(clutDefinition2.f37415id, clutDefinition2);
                }
                break;
            case 19:
                if (bits2 == subtitleService.subtitlePageId) {
                    ObjectData objectData = parseObjectData(parsableBitArray);
                    subtitleService.objects.put(objectData.f37416id, objectData);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ObjectData objectData2 = parseObjectData(parsableBitArray);
                    subtitleService.ancillaryObjects.put(objectData2.f37416id, objectData2);
                }
                break;
            case 20:
                if (bits2 == subtitleService.subtitlePageId) {
    ...
    public List<Cue> decode(byte[] bArr, int i) {
        ...
        for (int i2 = 0; i2 < sparseArray.size(); i2++) {
            ...
            while (i5 < sparseArray2.size()) {
                ...
                if (objectData != null) {
                    paintPixelDataSubBlocks(objectData, clutDefinition2, regionComposition.depth, i3 + regionObjectValueAt.horizontalPosition, i4 + regionObjectValueAt.verticalPosition, objectData.nonModifyingColorFlag ? null : this.defaultPaint, this.canvas);
                }
                i5++;
