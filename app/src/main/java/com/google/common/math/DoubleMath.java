...
public final class DoubleMath {
    ...
    static double roundIntermediate(double x, RoundingMode mode) {
        ...
        switch (C104891.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            case 1:
                MathPreconditions.checkRoundingUnnecessary(isMathematicalInteger(x));
                return x;
            case 2:
                return (x >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || isMathematicalInteger(x)) ? x : ((long) x) - 1;
            case 3:
                return (x <= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || isMathematicalInteger(x)) ? x : ((long) x) + 1;
            case 4:
                return x;
            case 5:
                ...
                return ((long) x) + ((long) (x > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE ? 1 : -1));
            case 6:
                return Math.rint(x);
            case 7:
    ...
    static /* synthetic */ class C104891 {
    ...
    public static boolean isPowerOfTwo(double x) {
        if (x > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE && DoubleUtils.isFinite(x)) {
    ...
    public static int log2(double x, RoundingMode mode) {
        ...
        boolean z = false;
        Preconditions.checkArgument(x > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE && DoubleUtils.isFinite(x), "x must be positive and finite");
    ...
    public static boolean isMathematicalInteger(double x) {
        if (DoubleUtils.isFinite(x)) {
            return x == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || 52 - Long.numberOfTrailingZeros(DoubleUtils.getSignificand(x)) <= Math.getExponent(x);
        }
        return false;
    }

    public static double factorial(int n) {
        MathPreconditions.checkNonNegative(C11341A5.f23826q, n);
        if (n > MAX_FACTORIAL) {
