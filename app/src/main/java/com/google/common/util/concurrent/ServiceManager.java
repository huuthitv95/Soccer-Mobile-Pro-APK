...
public final class ServiceManager implements ServiceManagerBridge {
    ...
    public ServiceManager(Iterable<? extends Service> services) {
        ...
        if (immutableListCopyOf.isEmpty()) {
            ...
            immutableListCopyOf = ImmutableList.m22833of(new NoOpService());
        }
        ...
        void awaitHealthy(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.awaitHealthGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.m22806in(ImmutableSet.m22881of(Service.State.NEW, Service.State.STARTING))));
                }
                checkHealthy();
                this.monitor.leave();
            } catch (Throwable th) {
        ...
        void awaitStopped(long timeout, TimeUnit unit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.stoppedGuard, timeout, unit)) {
                    throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.not(Predicates.m22806in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
                }
                this.monitor.leave();
            } catch (Throwable th) {
                this.monitor.leave();
