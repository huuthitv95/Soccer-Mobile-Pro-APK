...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N nodeU, N nodeV, E edge) {
        ...
        if (containsEdge(edge)) {
            ...
            EndpointPair endpointPairM22948of = EndpointPair.m22948of(this, nodeU, nodeV);
            Preconditions.checkArgument(endpointPairIncidentNodes.equals(endpointPairM22948of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", edge, endpointPairIncidentNodes, endpointPairM22948of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m22945of();
            }
            return DirectedNetworkConnections.m22946of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m22952of();
        }
        return UndirectedNetworkConnections.m22953of();
    }
}
