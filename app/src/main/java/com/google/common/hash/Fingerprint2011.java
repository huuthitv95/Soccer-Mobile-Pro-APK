...
final class Fingerprint2011 extends AbstractNonStreamingHashFunction {
    ...
    private static final long f22535K0 = -6505348102511208375L;

    /* JADX INFO: renamed from: K1 */
    private static final long f22536K1 = -8261664234251669945L;

    /* JADX INFO: renamed from: K2 */
    private static final long f22537K2 = -4288712594273399085L;

    /* JADX INFO: renamed from: K3 */
    private static final long f22538K3 = -4132994306676758123L;

    static long hash128to64(long high, long low) {
        long j = (low ^ high) * f22538K3;
        long j2 = (high ^ (j ^ (j >>> 47))) * f22538K3;
        return (j2 ^ (j2 >>> 47)) * f22538K3;
    }
    ...
    private static long fullFingerprint(byte[] bytes, int offset, int length) {
        ...
        long jLoad64 = LittleEndianByteArray.load64(bytes, offset);
        int i = offset + length;
        long jLoad65 = LittleEndianByteArray.load64(bArr, i - 16) ^ f22536K1;
        long jLoad66 = f22535K0 ^ LittleEndianByteArray.load64(bArr, i - 56);
        long[] jArr = new long[2];
        long[] jArr2 = new long[2];
        long j = length;
        weakHashLength32WithSeeds(bArr, i - 64, j, jLoad65, jArr);
        weakHashLength32WithSeeds(bArr, i - 32, j * f22536K1, f22535K0, jArr2);
        long[] jArr3 = jArr2;
        long jShiftMix = jLoad66 + (shiftMix(jArr[1]) * f22536K1);
        long jRotateRight = Long.rotateRight(jLoad64 + jShiftMix, 39) * f22536K1;
        int i2 = (length - 1) & (-64);
        long jRotateRight2 = Long.rotateRight(jLoad65, 33) * f22536K1;
        long j2 = jRotateRight;
        int i3 = offset;
        while (true) {
            long jRotateRight3 = Long.rotateRight(j2 + jRotateRight2 + jArr[0] + LittleEndianByteArray.load64(bArr, i3 + 16), 37) * f22536K1;
            long jRotateRight4 = Long.rotateRight(jRotateRight2 + jArr[1] + LittleEndianByteArray.load64(bArr, i3 + 48), 42) * f22536K1;
            long j3 = jArr3[1] ^ jRotateRight3;
            long j4 = jRotateRight4 ^ jArr[0];
            long jRotateRight5 = Long.rotateRight(jArr3[0] ^ jShiftMix, 33);
            weakHashLength32WithSeeds(bArr, i3, jArr[1] * f22536K1, jArr3[0] + j3, jArr);
            int i4 = i3;
            long[] jArr4 = jArr3;
            jRotateRight2 = j4;
            ...
            i2 -= 64;
            if (i2 == 0) {
                return hash128to64(hash128to64(jArr[0], jArr4[0]) + (shiftMix(jRotateRight2) * f22536K1) + j3, hash128to64(jArr[1], jArr4[1]) + jRotateRight5);
            }
            bArr = bytes;
            jArr3 = jArr4;
            j2 = jRotateRight5;
    ...
    private static long hashLength33To64(byte[] bytes, int offset, int length) {
        ...
        int i = offset + length;
        int i2 = i - 16;
        long jLoad65 = LittleEndianByteArray.load64(bytes, offset) + ((((long) length) + LittleEndianByteArray.load64(bytes, i2)) * f22535K0);
        ...
        return shiftMix((shiftMix(((j + jRotateRight5 + Long.rotateRight(jLoad611, 31) + jRotateRight7) * f22537K2) + ((jLoad69 + jLoad611 + jRotateRight4) * f22535K0)) * f22535K0) + jRotateRight4) * f22537K2;
    }

    static long murmurHash64WithSeed(byte[] bytes, int offset, int length, long seed) {
        int i = length & (-8);
        int i2 = length & 7;
        long jLoad64Safely = seed ^ (((long) length) * f22538K3);
        for (int i3 = 0; i3 < i; i3 += 8) {
            jLoad64Safely = (jLoad64Safely ^ (shiftMix(LittleEndianByteArray.load64(bytes, offset + i3) * f22538K3) * f22538K3)) * f22538K3;
        }
        if (i2 != 0) {
            jLoad64Safely = (LittleEndianByteArray.load64Safely(bytes, offset + i, i2) ^ jLoad64Safely) * f22538K3;
        }
        return shiftMix(shiftMix(jLoad64Safely) * f22538K3);
    }
}
