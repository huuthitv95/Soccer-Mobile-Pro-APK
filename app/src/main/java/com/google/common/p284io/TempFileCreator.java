...
abstract class TempFileCreator {
    ...
    static final class JavaNioCreator extends TempFileCreator {
        ...
        @Override // com.google.common.p284io.TempFileCreator
        File createTempDir() {
            try {
        ...
        public static PermissionSupplier userPermissions() {
            try {
                final ImmutableList immutableListM22833of = ImmutableList.m22833of(AclEntry.newBuilder().setType(AclEntryType.ALLOW).setPrincipal(FileSystems.getDefault().getUserPrincipalLookupService().lookupPrincipalByName(getUsername())).setPermissions((Set<AclEntryPermission>) EnumSet.allOf(ab$$ExternalSyntheticApiModelOutline0.m42853m())).setFlags(AclEntryFlag.DIRECTORY_INHERIT, AclEntryFlag.FILE_INHERIT).build());
