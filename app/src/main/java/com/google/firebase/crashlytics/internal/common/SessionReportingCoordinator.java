...
public class SessionReportingCoordinator {
    ...
    public void persistFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m23026v("Persisting fatal event for session " + str);
    ...
    public void persistNonFatalEvent(Throwable th, Thread thread, EventMetadata eventMetadata) {
        Logger.getLogger().m23026v("Persisting non-fatal event for session " + eventMetadata.getSessionId());
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (applicationExitInfoFindRelevantApplicationExitInfo == null) {
            Logger.getLogger().m23026v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m23020d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m23020d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsCustomKeysAndEventKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata, Map<String, String> map) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m23026v("No log data to include with this event.");
        }
        ...
        if (!z) {
            this.crashlyticsWorkers.diskWrite.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.SessionReportingCoordinator$$ExternalSyntheticLambda1
                ...
                public final void run() {
                    this.f$0.m23042x631b9fbf(eventAddMetaDataToEvent, eventMetadata, zEquals);
                }
            });
        } else {
    ...
    /* synthetic */ void m23042x631b9fbf(CrashlyticsReport.Session.Event event, EventMetadata eventMetadata, boolean z) {
        Logger.getLogger().m23020d("disk worker: log non-fatal event to persistence");
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger.getLogger().m23020d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger.getLogger().m23020d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger.getLogger().m23028w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m23029w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m23028w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
    ...
    private ApplicationExitInfo findRelevantApplicationExitInfo(String str, List<ApplicationExitInfo> list) {
        ...
        while (it.hasNext()) {
            ApplicationExitInfo applicationExitInfoM2264m = d$$ExternalSyntheticApiModelOutline0.m2264m((Object) it.next());
            if (applicationExitInfoM2264m.getTimestamp() < startTimestampMillis) {
                return null;
            }
            if (applicationExitInfoM2264m.getReason() == 6) {
                return applicationExitInfoM2264m;
            }
        }
        return null;
    }
