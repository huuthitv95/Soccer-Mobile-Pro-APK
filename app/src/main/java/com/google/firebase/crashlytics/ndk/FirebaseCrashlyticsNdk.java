...
class FirebaseCrashlyticsNdk implements CrashlyticsNativeComponent {
            ...
            try {
                ...
                SignalHandlerInstaller signalHandlerInstaller = new SignalHandlerInstaller() { // from class: com.google.firebase.crashlytics.ndk.FirebaseCrashlyticsNdk$$ExternalSyntheticLambda0
                    ...
                    public final void installHandler() {
                        this.f$0.m23049x251e82b0(str, str2, j, staticSessionData);
                    }
                };
    ...
    /* synthetic */ void m23049x251e82b0(String str, String str2, long j, StaticSessionData staticSessionData) {
        Logger.getLogger().m23020d("Initializing native session: " + str);
        if (this.controller.initialize(str, str2, j, staticSessionData)) {
            return;
        }
        Logger.getLogger().m23028w("Failed to initialize Crashlytics NDK for session " + str);
    }
    ...
    public synchronized void installSignalHandler() {
        ...
        if (this.installHandlerDuringPrepareSession) {
            Logger.getLogger().m23028w("Native signal handler already installed; skipping re-install.");
        } else {
            Logger.getLogger().m23020d("Deferring signal handler installation until the FirebaseCrashlyticsNdk session has been prepared");
