...
class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) throws Throwable {
        try {
            try {
                Logger.getLogger().m23020d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
                ...
                Task taskSubmitTask = this.crashlyticsWorkers.common.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                    ...
                    public Task<Void> call() throws Exception {
                        ...
                        if (currentSessionId != null) {
                            ...
                            return settingsProvider.getSettingsAsync().onSuccessTask(CrashlyticsController.this.crashlyticsWorkers.common, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                                ...
                                public Task<Void> then(Settings settings) throws Exception {
                                    if (settings == null) {
                                        Logger.getLogger().m23028w("Received null app settings, cannot send reports at crash time.");
                                        return Tasks.forResult(null);
                                    }
                        ...
                        Logger.getLogger().m23022e("Tried to write a fatal exception while no session was open.");
                        return Tasks.forResult(null);
                    }
                });
                if (!z) {
                    try {
                        ...
                        Logger.getLogger().m23022e("Cannot send reports. Timed out while fetching settings.");
                    } catch (Exception e) {
                        Logger.getLogger().m23023e("Error handling uncaught exception", e);
                    }
                }
            } catch (Throwable th2) {
                th = th2;
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m23020d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m23020d("Automatic data collection is disabled.");
        Logger.getLogger().m23026v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m23020d("Waiting for send/deleteUnsentReports to be called.");
    ...
    boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m23026v("Found previous crash marker.");
    ...
    Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m23028w("checkForUnsentReports should only be called once per execution.");
    ...
    void submitAllReports(final Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m23026v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
        } else {
            Logger.getLogger().m23026v("Crash reports are available to be sent.");
            waitForReportAction().onSuccessTask(this.crashlyticsWorkers.common, new SuccessContinuation<Boolean, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4
                ...
                public Task<Void> then(Boolean bool) throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m23026v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m23020d("Sending cached crash reports...");
                    ...
                    return task.onSuccessTask(CrashlyticsController.this.crashlyticsWorkers.common, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                        ...
                        public Task<Void> then(Settings settings) throws Exception {
                            ...
                            Logger.getLogger().m23028w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
    ...
    void writeNonFatalException(Thread thread, Throwable th, Map<String, String> map) {
        ...
        String currentSessionId = getCurrentSessionId();
        if (currentSessionId == null) {
            Logger.getLogger().m23028w("Tried to write a non-fatal exception while no session was open.");
        } else {
    ...
    void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m23028w("settingsProvider not set");
        } else {
    ...
    void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m23022e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    boolean finalizeSessions(SettingsProvider settingsProvider) {
        CrashlyticsWorkers.checkBackgroundThread();
        if (isHandlingException()) {
            Logger.getLogger().m23028w("Skipping session finalization because a crash has already occurred.");
            return Boolean.FALSE.booleanValue();
        }
        Logger.getLogger().m23026v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider, true);
            Logger.getLogger().m23026v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m23023e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m23020d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider, boolean z2) {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m23026v("No open sessions to be closed.");
            return;
        }
        ...
        if (z2 && settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m23026v("ANR feature disabled.");
        }
    ...
    void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m23024i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m23029w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        if (versionControlInfo != null) {
            Logger.getLogger().m23020d("Read version control info from string resource");
        ...
        if (resourceAsStream != null) {
            try {
                Logger.getLogger().m23020d("Read version control info from file");
        ...
        Logger.getLogger().m23024i("No version control information found");
        return null;
    }
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m23028w("Couldn't get Class Loader");
            return null;
        }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger.getLogger().m23026v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m23028w("No native core present");
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m23028w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m23020d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m23028w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m23024i("No Tombstones data found for session " + str);
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m23029w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m23028w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m23028w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m23020d("Logging app exception event to Firebase Analytics");
    ...
    static List<NativeSessionFile> getNativeSessionFiles(NativeSessionFileProvider nativeSessionFileProvider, String str, FileStore fileStore, byte[] bArr) {
        ...
        arrayList.add(new FileBackedNativeSessionFile("device_meta_file", C11744X3.i.f26328G, nativeSessionFileProvider.getDeviceFile()));
        arrayList.add(new FileBackedNativeSessionFile("os_meta_file", C11540L6.f24911F, nativeSessionFileProvider.getOsFile()));
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            if (historicalProcessExitReasons.size() != 0) {
                ...
                Logger.getLogger().m23026v("No ApplicationExitInfo available. Session: " + str);
                return;
            }
        }
        Logger.getLogger().m23026v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
