...
public class CrashpadController {
    ...
    public boolean initialize(String str, String str2, long j, StaticSessionData staticSessionData) throws Throwable {
        ...
        try {
            ...
            Logger.getLogger().m23023e("Error initializing Crashlytics NDK", e);
            return false;
        }
    }
    ...
    public SessionFiles getFilesForSession(String str) {
        ...
        Logger.getLogger().m23026v("Minidump directory: " + file.getAbsolutePath());
        File singleFileWithExtension = getSingleFileWithExtension(file, ".dmp");
        Logger.getLogger().m23026v("Minidump file ".concat((singleFileWithExtension == null || !singleFileWithExtension.exists()) ? "does not exist" : "exists"));
    ...
    private CrashlyticsReport.ApplicationExitInfo getRelevantApplicationExitInfo(long j, List<ApplicationExitInfo> list) {
        ...
        while (it.hasNext()) {
            ...
            if (applicationExitInfoM2264m.getReason() == 5 && applicationExitInfoM2264m.getTimestamp() >= j) {
                arrayList.add(applicationExitInfoM2264m);
            }
        }
        if (arrayList.isEmpty()) {
            return null;
        }
        return convertApplicationExitInfoToModel(d$$ExternalSyntheticApiModelOutline0.m2264m(arrayList.get(0)));
    }
    ...
    private static String getTraceFileFromApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        try {
            ...
            Logger.getLogger().m23028w("Failed to get input stream from ApplicationExitInfo");
            return null;
        }
    }
