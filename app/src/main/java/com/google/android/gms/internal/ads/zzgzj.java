...
public final class zzgzj {
    ...
    public static boolean zzb(double d) {
        if (d > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE && zzgzk.zzb(d)) {
            long jZza = zzgzk.zza(d);
            if ((jZza & ((-1) + jZza)) == 0) {
                return true;
            }
        }
    ...
    public static int zzc(double d, RoundingMode roundingMode) {
        boolean zZzb;
        boolean z = false;
        zzgsw.zzb(d > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE && zzgzk.zzb(d), "x must be positive and finite");
    ...
    public static boolean zzd(double d) {
        if (zzgzk.zzb(d)) {
            return d == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || 52 - Long.numberOfTrailingZeros(zzgzk.zza(d)) <= Math.getExponent(d);
        }
        return false;
    }
}
