...
public final class DownloadManager {
    ...
    static final class InternalHandler extends Handler {
        ...
        private void addDownload(DownloadRequest downloadRequest, int i) {
            Download download = getDownload(downloadRequest.f21864id, true);
        ...
        private Download getDownload(String str, boolean z) {
            ...
            try {
                ...
                Log.m22237e(DownloadManager.TAG, "Failed to load download: " + str, e);
                return null;
            }
        }
        ...
        private int getDownloadIndex(String str) {
            for (int i = 0; i < this.downloads.size(); i++) {
                if (this.downloads.get(i).request.f21864id.equals(str)) {
                    return i;
                }
            }
            return -1;
        ...
        private void initialize(int i) {
            ...
            try {
                try {
                    ...
                    Log.m22237e(DownloadManager.TAG, "Failed to load index.", e);
        ...
        private void onContentLengthChanged(Task task, long j) {
            Download download = (Download) Assertions.checkNotNull(getDownload(task.request.f21864id, false));
        ...
        private void onDownloadTaskStopped(Download download, Exception exc) {
            ...
            this.downloads.remove(getDownloadIndex(download2.request.f21864id));
            try {
                this.downloadIndex.putDownload(download2);
            } catch (IOException e) {
                Log.m22237e(DownloadManager.TAG, "Failed to update index.", e);
            }
        ...
        private void onRemoveTaskStopped(Download download) {
            if (download.state == 7) {
                ...
                this.downloads.remove(getDownloadIndex(download.request.f21864id));
                try {
                    this.downloadIndex.removeDownload(download.request.f21864id);
                } catch (IOException unused) {
                    Log.m22236e(DownloadManager.TAG, "Failed to remove from database");
                }
        ...
        private void onTaskStopped(Task task) {
            String str = task.request.f21864id;
            ...
            if (exc != null) {
                Log.m22237e(DownloadManager.TAG, "Task failed: " + task.request + ", " + z, exc);
            }
        ...
        private Download putDownload(Download download) {
            ...
            int downloadIndex = getDownloadIndex(download.request.f21864id);
            if (downloadIndex == -1) {
        ...
        private void removeAllDownloads() {
            ...
            try {
                ...
                Log.m22236e(DownloadManager.TAG, "Failed to load downloads.");
            }
            for (int i = 0; i < this.downloads.size(); i++) {
        ...
        private void removeDownload(String str) {
            ...
            if (download != null) {
                ...
                Log.m22236e(DownloadManager.TAG, "Failed to remove nonexistent download: " + str);
            }
        }
        ...
        private void setStopReason(String str, int i) {
            if (str == null) {
                ...
                try {
                    ...
                    Log.m22237e(DownloadManager.TAG, "Failed to set manual stop reason", e);
                }
            } else {
                ...
                if (download != null) {
                    ...
                    try {
                        ...
                        Log.m22237e(DownloadManager.TAG, "Failed to set manual stop reason: " + str, e2);
                    }
                }
            }
            syncTasks();
        ...
        private Task syncQueuedDownload(Task task, Download download) {
            ...
            this.activeTasks.put(downloadPutDownloadWithState.request.f21864id, task2);
        ...
        private void syncRemovingDownload(Task task, Download download) {
            if (task != null) {
                ...
                this.activeTasks.put(download.request.f21864id, task2);
        ...
        private void syncTasks() {
            ...
            for (int i2 = 0; i2 < this.downloads.size(); i2++) {
                ...
                Task taskSyncQueuedDownload = this.activeTasks.get(download.request.f21864id);
