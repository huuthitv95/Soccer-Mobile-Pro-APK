...
public final class MediaMetricsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    public static MediaMetricsListener create(Context context) {
        ...
        return new MediaMetricsListener(context, mediaMetricsManagerM435m.createPlaybackSession());
    }
    ...
    private static int getDrmType(DrmInitData drmInitData) {
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if (uuid.equals(C9415C.WIDEVINE_UUID)) {
                return 3;
            }
            if (uuid.equals(C9415C.PLAYREADY_UUID)) {
                return 2;
            }
            if (uuid.equals(C9415C.CLEARKEY_UUID)) {
                return 6;
            }
        }
        return 1;
    ...
    private void maybeReportNetworkChange(long j) {
        ...
        if (networkType != this.currentNetworkType) {
            ...
            this.playbackSession.reportNetworkEvent(FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m436m().setNetworkType(networkType).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void maybeReportPlaybackError(long j) {
        ...
        this.playbackSession.reportPlaybackErrorEvent(FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m440m().setTimeSinceCreatedMillis(j - this.startTimeMs).setErrorCode(errorInfo.errorCode).setSubErrorCode(errorInfo.subErrorCode).setException(playbackException).build());
    ...
    private void maybeReportPlaybackStateChange(Player player, AnalyticsListener.Events events, long j) {
        ...
        if (this.currentPlaybackState != iResolveNewPlaybackState) {
            ...
            this.reportedEventsForCurrentSession = true;
            this.playbackSession.reportPlaybackStateEvent(FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m452m().setState(this.currentPlaybackState).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void maybeUpdateMetricsBuilderValues(Player player, AnalyticsListener.Events events) {
        ...
        if (events.contains(2) && this.metricsBuilder != null && (drmInitData = getDrmInitData(player.getCurrentTracks().getGroups())) != null) {
            FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m449m(Util.castNonNull(this.metricsBuilder)).setDrmType(getDrmType(drmInitData));
        }
    ...
    private void reportTrackChangeEvent(int i, long j, Format format, int i2) {
        TrackChangeEvent.Builder timeSinceCreatedMillis = FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m456m(i).setTimeSinceCreatedMillis(j - this.startTimeMs);
        if (format != null) {
    ...
    public void onSessionActive(AnalyticsListener.EventTime eventTime, String str) {
        if (eventTime.mediaPeriodId == null || !eventTime.mediaPeriodId.isAd()) {
            ...
            this.metricsBuilder = FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m445m().setPlayerName(ExoPlayerLibraryInfo.TAG).setPlayerVersion(ExoPlayerLibraryInfo.VERSION);
            maybeUpdateTimelineMetadata(eventTime.timeline, eventTime.mediaPeriodId);
        }
    }
