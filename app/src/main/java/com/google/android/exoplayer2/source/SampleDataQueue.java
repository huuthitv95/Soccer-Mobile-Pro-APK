...
class SampleDataQueue {
    ...
    private static AllocationNode readEncryptionData(AllocationNode allocationNode, DecoderInputBuffer decoderInputBuffer, SampleQueue.SampleExtrasHolder sampleExtrasHolder, ParsableByteArray parsableByteArray) {
        ...
        if (cryptoInfo.f21850iv == null) {
            cryptoInfo.f21850iv = new byte[16];
        } else {
            Arrays.fill(cryptoInfo.f21850iv, (byte) 0);
        }
        AllocationNode data2 = readData(data, j2, cryptoInfo.f21850iv, i);
        long j3 = j2 + ((long) i);
        if (z) {
        ...
        cryptoInfo.set(i2, iArr2, iArr4, cryptoData.encryptionKey, cryptoInfo.f21850iv, cryptoData.cryptoMode, cryptoData.encryptedBlocks, cryptoData.clearBlocks);
