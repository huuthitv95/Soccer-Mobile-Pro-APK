...
public final class Cea708Decoder extends CeaDecoder {
    ...
    private static final class CueInfoBuilder {
        ...
        public Cea708CueInfo build() {
            ...
            return new Cea708CueInfo(spannableStringBuilder, alignment, f4, 0, i5, f3, i3, -3.4028235E38f, this.windowFillColor != COLOR_SOLID_BLACK, this.windowFillColor, this.priority);
        }
    ...
    private void handleC0Command(int i) {
        if (i != 0) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m22240w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.captionChannelPacketData.skipBits(8);
                    } else if (i >= 24 && i <= 31) {
                        Log.m22240w(TAG, "Currently unsupported COMMAND_P16 Command: " + i);
                        this.captionChannelPacketData.skipBits(16);
                    } else {
                        Log.m22240w(TAG, "Invalid C0 command: " + i);
                    }
                    break;
            }
        }
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            case 150:
            default:
                Log.m22240w(TAG, "Invalid C1 command: " + i);
                break;
            ...
            default:
                switch (i) {
                    ...
                    case 127:
                        ...
                        break;
                    default:
                        Log.m22240w(TAG, "Invalid G2 character: " + i);
                        break;
                }
                break;
        }
    ...
    private void handleG3Character(int i) {
        ...
        Log.m22240w(TAG, "Invalid G3 character: " + i);
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.m22234d(TAG, "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + ");");
        }
        ...
        while (this.captionChannelPacketData.bitsLeft() > 0) {
            ...
            if (bits == 7) {
                ...
                if (bits < 7) {
                    Log.m22240w(TAG, "Invalid extended service number: " + bits);
                }
            }
            if (bits2 == 0) {
                if (bits == 0) {
                ...
                Log.m22240w(TAG, "serviceNumber is non-zero (" + bits + ") when blockSize is 0");
                break;
            }
                    ...
                    if (bits3 == 16) {
                        ...
                        if (bits4 <= 31) {
                            ...
                            if (bits4 <= 127) {
                                ...
                                Log.m22240w(TAG, "Invalid extended command: " + bits4);
                            }
                            z = true;
                        }
                    } else if (bits3 <= 31) {
                        ...
                        if (bits3 <= 127) {
                            ...
                            Log.m22240w(TAG, "Invalid base command: " + bits3);
                        }
                        z = true;
                    }
                }
            ...
            if (i == 2 || i == 3) {
                if (z) {
                    if (i == 3) {
                        ...
                        if (i3 != -1 && i2 != (i3 + 1) % 4) {
                            resetCueBuilders();
                            Log.m22240w(TAG, "Sequence number discontinuity. previous=" + this.previousSequenceNumber + " current=" + i2);
                        }
                        ...
                        if (dtvCcPacket3 == null) {
                            Log.m22236e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                        } else {
