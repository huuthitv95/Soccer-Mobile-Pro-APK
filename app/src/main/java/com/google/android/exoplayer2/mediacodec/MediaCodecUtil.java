...
public final class MediaCodecUtil {
    ...
    private static Pair<Integer, Integer> getAacCodecProfileAndLevel(String str, String[] strArr) {
        ...
        if (strArr.length != 3) {
            Log.m22240w(TAG, "Ignoring malformed MP4A codec string: " + str);
            return null;
        }
        try {
    ...
    private static Pair<Integer, Integer> getAv1ProfileAndLevel(String str, String[] strArr, ColorInfo colorInfo) {
        if (strArr.length < 4) {
            Log.m22240w(TAG, "Ignoring malformed AV1 codec string: " + str);
            return null;
        }
        int i = 1;
        try {
            ...
            if (i2 != 0) {
                Log.m22240w(TAG, "Unknown AV1 profile: " + i2);
                return null;
            }
            if (i4 != 8 && i4 != 10) {
                Log.m22240w(TAG, "Unknown AV1 bit depth: " + i4);
                return null;
            }
            if (i4 != 8) {
            ...
            Log.m22240w(TAG, "Unknown AV1 level: " + i3);
            return null;
        } catch (NumberFormatException unused) {
            Log.m22240w(TAG, "Ignoring malformed AV1 codec string: " + str);
            return null;
    ...
    private static Pair<Integer, Integer> getAvcProfileAndLevel(String str, String[] strArr) {
        ...
        int i2;
        if (strArr.length < 2) {
            Log.m22240w(TAG, "Ignoring malformed AVC codec string: " + str);
            return null;
        }
        try {
            ...
            if (iAvcProfileNumberToConst == -1) {
                Log.m22240w(TAG, "Unknown AVC profile: " + i2);
                return null;
            }
            ...
            Log.m22240w(TAG, "Unknown AVC level: " + i);
            return null;
        } catch (NumberFormatException unused) {
            Log.m22240w(TAG, "Ignoring malformed AVC codec string: " + str);
            return null;
    ...
    public static synchronized List<MediaCodecInfo> getDecoderInfos(String str, boolean z, boolean z2) throws DecoderQueryException {
        ...
        if (z && decoderInfosInternal.isEmpty() && 21 <= Util.SDK_INT && Util.SDK_INT <= 23) {
            ...
            if (!decoderInfosInternal.isEmpty()) {
                Log.m22240w(TAG, "MediaCodecList API didn't list secure decoder for: " + str + ". Assuming: " + decoderInfosInternal.get(0).name);
            }
        }
  ...
  0x008f: PHI (r16v9 boolean) = (r16v5 boolean), (r16v11 boolean) binds: [B:41:0x0097, B:35:0x008c] A[DONT_GENERATE, DONT_INLINE]] */
    /* JADX WARN: Code duplicated, block: B:68:0x00fd A[Catch: Exception -> 0x0147, TRY_ENTER, TryCatch #5 {Exception -> 0x0147, blocks: (B:3:0x0008, B:5:0x001b, B:71:0x011c, B:8:0x002b, B:11:0x0036, B:65:0x00f5, B:68:0x00fd, B:70:0x0103, B:72:0x0124, B:73:0x0145), top: B:88:0x0008 }] */
    /* JADX WARN: Code duplicated, block: B:94:0x0124 A[ADDED_TO_REGION, REMOVE, SYNTHETIC] */
                                        ...
                                        try {
                                            ...
                                            try {
                                                ...
                                                if (z3) {
                                                    ...
                                                    if (codecKey2.secure == zIsFeatureSupported2) {
                                                        ...
                                                        try {
                                                            ...
                                                            Log.m22236e(TAG, "Failed to query codec " + r6 + " (" + codecMimeType + ")");
                                                            throw e;
                                                        }
                                                    }
                                                } else {
                                                ...
                                                if (!z3) {
                                                    try {
                                                        if (!codecKey2.secure) {
                                                            ...
                                                            boolean z5 = z2;
                                                            i = i3;
                                                            arrayList.add(MediaCodecInfo.newInstance(name, str, codecMimeType, capabilitiesForType, zIsHardwareAccelerated, zIsSoftwareOnly, z5, false, false));
                                                        }
                                                    } catch (Exception e2) {
                                                        e = e2;
                                                        i = i3;
                                                ...
                                                if (!z && zIsFeatureSupported2) {
                                                    try {
                                                        arrayList.add(MediaCodecInfo.newInstance(name + ".secure", str, codecMimeType, capabilitiesForType, zIsHardwareAccelerated, zIsSoftwareOnly, z6, false, true));
                                                        break;
                                                    } catch (Exception e3) {
                                                        e = e3;
                                                        ...
                                                        Log.m22236e(TAG, "Skipping codec " + name + " (failed to query capabilities)");
                                                        i2 = i + 1;
                                                        codecKey2 = codecKey;
    ...
    private static Pair<Integer, Integer> getDolbyVisionProfileAndLevel(String str, String[] strArr) {
        if (strArr.length < 3) {
            Log.m22240w(TAG, "Ignoring malformed Dolby Vision codec string: " + str);
            return null;
        }
        ...
        if (numDolbyVisionStringToProfile == null) {
            Log.m22240w(TAG, "Unknown Dolby Vision profile string: " + strGroup);
            return null;
        }
        ...
        Log.m22240w(TAG, "Unknown Dolby Vision level string: " + str2);
        return null;
    }

    private static Pair<Integer, Integer> getHevcProfileAndLevel(String str, String[] strArr, ColorInfo colorInfo) {
        if (strArr.length < 4) {
            Log.m22240w(TAG, "Ignoring malformed HEVC codec string: " + str);
            return null;
        }
        int i = 1;
        Matcher matcher = PROFILE_PATTERN.matcher(strArr[1]);
        ...
        if (!"1".equals(strGroup)) {
            if (!"2".equals(strGroup)) {
                Log.m22240w(TAG, "Unknown HEVC profile string: " + strGroup);
                return null;
            }
        ...
        Log.m22240w(TAG, "Unknown HEVC level string: " + str2);
        return null;
    }

    private static Pair<Integer, Integer> getVp9ProfileAndLevel(String str, String[] strArr) {
        if (strArr.length < 3) {
            Log.m22240w(TAG, "Ignoring malformed VP9 codec string: " + str);
            return null;
        }
        try {
            ...
            if (iVp9ProfileNumberToConst == -1) {
                Log.m22240w(TAG, "Unknown VP9 profile: " + i);
                return null;
            }
            ...
            Log.m22240w(TAG, "Unknown VP9 level: " + i2);
            return null;
        } catch (NumberFormatException unused) {
            Log.m22240w(TAG, "Ignoring malformed VP9 codec string: " + str);
            return null;
    ...
    public static void warmDecoderInfoCache(String str, boolean z, boolean z2) {
        try {
            ...
            Log.m22237e(TAG, "Codec warming failed", e);
        }
    }
}
