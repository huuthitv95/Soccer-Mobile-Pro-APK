    ...
    private static DrmInitData getDrmInitDataFromAtoms(List<Atom.LeafAtom> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (leafAtom.type == 1886614376) {
                ...
                if (uuid == null) {
                    Log.m22240w(TAG, "Skipped pssh atom (failed to extract uuid)");
                } else {
    ...
    private void onEmsgLeafAtomRead(ParsableByteArray parsableByteArray) {
        ...
        if (fullAtomVersion == 0) {
            ...
            if (fullAtomVersion != 1) {
                Log.m22240w(TAG, "Skipping unsupported emsg version: " + fullAtomVersion);
                return;
            }
    ...
    private void onMoovContainerAtomRead(Atom.ContainerAtom containerAtom) throws ParserException {
        ...
        if (this.trackBundles.size() != 0) {
            ...
            while (i < size2) {
                ...
                this.trackBundles.get(track.f21852id).reset(trackSampleTable, getDefaultSampleValues(sparseArray, track.f21852id));
                i++;
            }
            return;
        }
        while (i < size2) {
            ...
            this.trackBundles.put(track2.f21852id, new TrackBundle(this.extractorOutput.track(i, track2.type), trackSampleTable2, getDefaultSampleValues(sparseArray, track2.f21852id)));
    ...
    private boolean readSample(ExtractorInput extractorInput) throws IOException {
        ...
        if (nextTrackBundle == null) {
            ...
            if (currentSampleOffset < 0) {
                Log.m22240w(TAG, "Ignoring negative offset to sample data.");
