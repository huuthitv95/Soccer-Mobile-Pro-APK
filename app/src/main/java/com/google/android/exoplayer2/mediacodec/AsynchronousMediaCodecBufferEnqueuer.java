...
class AsynchronousMediaCodecBufferEnqueuer {
    ...
    private static void copy(CryptoInfo cryptoInfo, MediaCodec.CryptoInfo cryptoInfo2) {
        ...
        cryptoInfo2.iv = (byte[]) Assertions.checkNotNull(copy(cryptoInfo.f21850iv, cryptoInfo2.iv));
        cryptoInfo2.mode = cryptoInfo.mode;
        if (Util.SDK_INT >= 24) {
            FrameworkMediaDrm$$ExternalSyntheticApiModelOutline0.m469m();
            cryptoInfo2.setPattern(Util$$ExternalSyntheticApiModelOutline0.m320m(cryptoInfo.encryptedBlocks, cryptoInfo.clearBlocks));
        }
    }
    ...
    public void doHandleMessage(Message message) {
        ...
        if (i != 0) {
            if (i != 1) {
                messageParams2 = null;
                if (i != 2) {
                    LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m280m(this.pendingRuntimeException, null, new IllegalStateException(String.valueOf(message.what)));
                } else {
    ...
    private void doQueueInputBuffer(int i, int i2, int i3, long j, int i4) {
        try {
            ...
            LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m280m(this.pendingRuntimeException, null, e);
        }
    }
