...
import com.google.zxing.pdf417.decoder.p285ec.ErrorCorrection;
        ...
        while (i8 <= barcodeColumnCount) {
            ...
            if (detectionResult.getDetectionResultColumn(i9) == null) {
                if (i9 == 0 || i9 == barcodeColumnCount) {
                    detectionResultRowIndicatorColumn = new DetectionResultRowIndicatorColumn(boundingBox, i9 == 0 ? true : z);
                } else {
                    detectionResultRowIndicatorColumn = new DetectionResultColumn(boundingBox);
                }
                detectionResult.setDetectionResultColumn(i9, detectionResultRowIndicatorColumn);
                int i10 = -1;
                int iMin = i6;
                int iMax = i7;
                ...
                while (minY <= boundingBox.getMaxY()) {
                    int startColumn = getStartColumn(detectionResult, i9, minY, z2);
                    if (startColumn >= 0 && startColumn <= boundingBox.getMaxX()) {
                        detectionResultColumn = detectionResultRowIndicatorColumn;
                        i3 = i10;
                        i4 = startColumn;
                        codewordDetectCodeword = detectCodeword(bitMatrix, boundingBox.getMinX(), boundingBox.getMaxX(), z2, i4, minY, iMin, iMax);
