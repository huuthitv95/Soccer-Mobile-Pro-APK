...
public final class DebugMetadataKt {
    ...
    public static final String[] getSpilledVariableFieldMapping(BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.m43510v());
        ArrayList arrayList = new ArrayList();
        int label = getLabel(baseContinuationImpl);
        int[] iArrM43505i = debugMetadataAnnotation.m43505i();
        int length = iArrM43505i.length;
        for (int i = 0; i < length; i++) {
            if (iArrM43505i[i] == label) {
                arrayList.add(debugMetadataAnnotation.m43509s()[i]);
                arrayList.add(debugMetadataAnnotation.m43508n()[i]);
            }
        }
    ...
    public static final StackTraceElement getStackTraceElement(BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.m43510v());
        int label = getLabel(baseContinuationImpl);
        int i = label < 0 ? -1 : debugMetadataAnnotation.m43506l()[label];
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName(baseContinuationImpl);
        if (moduleName == null) {
            strM43503c = debugMetadataAnnotation.m43503c();
        } else {
            strM43503c = moduleName + '/' + debugMetadataAnnotation.m43503c();
        }
        return new StackTraceElement(strM43503c, debugMetadataAnnotation.m43507m(), debugMetadataAnnotation.m43504f(), i);
    }
}
