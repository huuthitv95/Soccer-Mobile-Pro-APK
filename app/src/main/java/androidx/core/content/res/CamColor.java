...
public class CamColor {
    ...
    private final float f79mJ;
    private final float mJstar;

    /* JADX INFO: renamed from: mM */
    private final float f80mM;

    /* JADX INFO: renamed from: mQ */
    private final float f81mQ;

    /* JADX INFO: renamed from: mS */
    private final float f82mS;
    ...
    int viewed(ViewingConditions viewingConditions) {
        ...
        float fSignum3 = Math.signum(f7) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((float) Math.max(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, (((double) Math.abs(f7)) * 27.13d) / (400.0d - ((double) Math.abs(f7)))), 2.380952380952381d));
