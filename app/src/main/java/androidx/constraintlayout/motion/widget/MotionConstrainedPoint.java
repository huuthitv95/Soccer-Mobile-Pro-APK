...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    void different(MotionConstrainedPoint points, boolean[] mask, String[] custom) {
        ...
        mask[1] = mask[1] | diff(this.f64x, points.f64x);
        mask[2] = mask[2] | diff(this.f65y, points.f65y);
    ...
    void fillStandard(double[] data, int[] toUse) {
        int i = 0;
        float[] fArr = {this.position, this.f64x, this.f65y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        for (int i2 : toUse) {
            if (i2 < 18) {
                data[i] = fArr[i2];
                i++;
