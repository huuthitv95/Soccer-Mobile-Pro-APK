        ...
        try {
            ...
            if (z) {
                for (WorkSpec workSpec : runningWork) {
                    workSpecDao.setState(WorkInfo.State.ENQUEUED, workSpec.f194id);
                    workSpecDao.markWorkSpecScheduled(workSpec.f194id, -1L);
                }
            }
            ...
            if (Build.VERSION.SDK_INT >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    for (int i = 0; i < historicalProcessExitReasons.size(); i++) {
                        if (d$$ExternalSyntheticApiModelOutline0.m2264m(historicalProcessExitReasons.get(i)).getReason() == 10) {
                            return true;
                        }
                    }
                }
