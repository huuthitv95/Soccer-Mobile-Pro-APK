...
class SystemJobInfoConverter {
    ...
    private static JobInfo.TriggerContentUri convertContentUriTrigger(ContentUriTriggers.Trigger trigger) {
        ...
        return Trace$$ExternalSyntheticApiModelOutline0.m762m(trigger.getUri(), zShouldTriggerForDescendants ? 1 : 0);
    }

    static int convertNetworkType(NetworkType networkType) {
        int i = C11781.$SwitchMap$androidx$work$NetworkType[networkType.ordinal()];
        if (i == 1) {
            return 0;
        }
        if (i == 2) {
    ...
    JobInfo convert(WorkSpec workSpec, int jobId) {
        ...
        persistableBundle.putString(EXTRA_WORK_SPEC_ID, workSpec.f194id);
