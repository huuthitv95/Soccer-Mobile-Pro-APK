...
public class ChangeImageTransform extends Transition {
    ...
    private static class Listener extends AnimatorListenerAdapter implements Transition.TransitionListener {
        ...
        private void restoreMatrix() {
            Matrix matrix = (Matrix) this.mImageView.getTag(C1085R.id.transition_image_transform);
            if (matrix != null) {
                ImageViewUtils.animateTransform(this.mImageView, matrix);
                this.mImageView.setTag(C1085R.id.transition_image_transform, null);
            }
        }

        private void saveMatrix(Matrix matrix) {
            this.mImageView.setTag(C1085R.id.transition_image_transform, matrix);
    ...
    private void captureValues(TransitionValues transitionValues, boolean z) {
        ...
        if ((view instanceof ImageView) && view.getVisibility() == 0) {
            ...
            Matrix matrixCopyImageMatrix = z ? (Matrix) imageView.getTag(C1085R.id.transition_image_transform) : null;
    ...
    private static Matrix copyImageMatrix(ImageView imageView) {
        ...
        int i = C10753.$SwitchMap$android$widget$ImageView$ScaleType[imageView.getScaleType().ordinal()];
        if (i != 1) {
