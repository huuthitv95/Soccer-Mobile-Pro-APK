    ...
    */
        ...
        if (i == 2) {
            ...
            if (Util.SDK_INT >= 23) {
                try {
                    ...
                    Log.m292w(TAG, "Failed to update the DRM session, releasing the codec instead.", e);
                    releaseCodec();
                    return true;
                }
            }
    ...
    private void maybeInitCodecWithFallback(MediaCrypto mediaCrypto, boolean z) throws DecoderInitializationException {
        ...
        while (this.codec == null) {
            ...
            try {
                ...
                Log.m292w(TAG, "Failed to initialize decoder: " + mediaCodecInfo, e2);
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean z) throws MediaCodecUtil.DecoderQueryException {
        ...
        if (!decoderInfos2.isEmpty()) {
            Log.m291w(TAG, "Drm session requires secure decoder for " + format.sampleMimeType + ", but no secure decoder available. Trying to proceed with " + decoderInfos2 + ".");
        }
    ...
    private void initCodec(MediaCodecInfo mediaCodecInfo, MediaCrypto mediaCrypto) throws Exception {
        ...
        try {
            ...
            if (!mediaCodecInfo.isFormatSupported(format)) {
                Log.m291w(TAG, Util.formatInvariant("Format exceeds selected codec's capabilities [%s, %s]", Format.toLogString(format), str));
            }
            this.codecInfo = mediaCodecInfo;
    ...
    private boolean drmNeedsCodecReinitialization(MediaCodecInfo mediaCodecInfo, Format format, DrmSession drmSession, DrmSession drmSession2) throws ExoPlaybackException {
        ...
        if (drmSession2 != null && drmSession != null && (cryptoConfig = drmSession2.getCryptoConfig()) != null && (cryptoConfig2 = drmSession.getCryptoConfig()) != null && cryptoConfig.getClass().equals(cryptoConfig2.getClass())) {
            ...
            if (drmSession2.getSchemeUuid().equals(drmSession.getSchemeUuid()) && Util.SDK_INT >= 23 && !C0684C.PLAYREADY_UUID.equals(drmSession.getSchemeUuid()) && !C0684C.PLAYREADY_UUID.equals(drmSession2.getSchemeUuid())) {
