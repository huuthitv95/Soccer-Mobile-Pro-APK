...
class SampleDataQueue {
    ...
    private static AllocationNode readEncryptionData(AllocationNode allocationNode, DecoderInputBuffer decoderInputBuffer, SampleQueue.SampleExtrasHolder sampleExtrasHolder, ParsableByteArray parsableByteArray) {
        ...
        if (cryptoInfo.f149iv == null) {
            cryptoInfo.f149iv = new byte[16];
        } else {
            Arrays.fill(cryptoInfo.f149iv, (byte) 0);
        }
        AllocationNode data2 = readData(data, j2, cryptoInfo.f149iv, i);
        long j3 = j2 + ((long) i);
        if (z) {
        ...
        cryptoInfo.set(i2, iArr2, iArr4, cryptoData.encryptionKey, cryptoInfo.f149iv, cryptoData.cryptoMode, cryptoData.encryptedBlocks, cryptoData.clearBlocks);
