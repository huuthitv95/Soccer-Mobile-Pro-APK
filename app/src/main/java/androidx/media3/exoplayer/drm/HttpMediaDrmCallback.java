...
public final class HttpMediaDrmCallback implements MediaDrmCallback {
    ...
    public byte[] executeKeyRequest(UUID uuid, ExoMediaDrm.KeyRequest keyRequest) throws MediaDrmCallbackException {
        ...
        if (TextUtils.isEmpty(licenseServerUrl)) {
            throw new MediaDrmCallbackException(new DataSpec.Builder().setUri(Uri.EMPTY).build(), Uri.EMPTY, ImmutableMap.m22851of(), 0L, new IllegalStateException("No license URL"));
        }
        HashMap map = new HashMap();
        if (C0684C.PLAYREADY_UUID.equals(uuid)) {
            str = "text/xml";
        } else {
            str = C0684C.CLEARKEY_UUID.equals(uuid) ? C11341A5.f23800M : "application/octet-stream";
        }
