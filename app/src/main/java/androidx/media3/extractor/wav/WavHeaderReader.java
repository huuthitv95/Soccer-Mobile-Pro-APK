...
final class WavHeaderReader {
    ...
    public static boolean checkFileType(ExtractorInput extractorInput) throws IOException {
        ...
        if (chunkHeaderPeek.f179id != 1380533830 && chunkHeaderPeek.f179id != 1380333108) {
            return false;
        }
        ...
        Log.m287e(TAG, "Unsupported form type: " + i);
        return false;
    }
    ...
    public static long readRf64SampleDataSize(ExtractorInput extractorInput) throws IOException {
        ParsableByteArray parsableByteArray = new ParsableByteArray(8);
        ChunkHeader chunkHeaderPeek = ChunkHeader.peek(extractorInput, parsableByteArray);
        if (chunkHeaderPeek.f179id != 1685272116) {
    ...
    private static ChunkHeader skipToChunk(int i, ExtractorInput extractorInput, ParsableByteArray parsableByteArray) throws IOException {
        ChunkHeader chunkHeaderPeek = ChunkHeader.peek(extractorInput, parsableByteArray);
        while (chunkHeaderPeek.f179id != i) {
            Log.m291w(TAG, "Ignoring unknown WAV chunk: " + chunkHeaderPeek.f179id);
            ...
            if (j2 > 2147483647L) {
                throw ParserException.createForUnsupportedContainerFeature("Chunk is too large (~2GB+) to skip; id: " + chunkHeaderPeek.f179id);
            }
