...
public final class Tx3gParser implements SubtitleParser {
    ...
    public void parse(byte[] bArr, int i, int i2, SubtitleParser.OutputOptions outputOptions, Consumer<CuesWithTiming> consumer) {
        ...
        if (subtitleText.isEmpty()) {
            consumer.accept(new CuesWithTiming(ImmutableList.m22832of(), -9223372036854775807L, -9223372036854775807L));
            return;
        }
        ...
        consumer.accept(new CuesWithTiming(ImmutableList.m22833of(new Cue.Builder().setText(spannableStringBuilder).setLine(fConstrainValue, 0).setLineAnchor(0).build()), -9223372036854775807L, -9223372036854775807L));
    }
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) {
        ...
        if (unsignedShort2 > spannableStringBuilder.length()) {
            Log.m291w(TAG, "Truncating styl end (" + unsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            unsignedShort2 = spannableStringBuilder.length();
        }
        int i2 = unsignedShort2;
        if (unsignedShort >= i2) {
            Log.m291w(TAG, "Ignoring styl with start (" + unsignedShort + ") >= end (" + i2 + ").");
            return;
        }
